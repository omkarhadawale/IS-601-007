...
def importCSV():
    if request.method == "POST":
        ...
        if file and secure_filename(file.filename) and file_extention=='csv':
            companies = []
            employees = []
            company_query = """
            ...
            if len(companies) > 0:
                ...
                flash("No company records were added into Database from csv file",'warning')
            if len(employees) > 0:
                ...
                flash("No Employee records were added into Database from csv file",'warning')
